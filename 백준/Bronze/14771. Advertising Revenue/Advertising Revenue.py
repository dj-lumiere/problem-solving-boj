...
with open(0, 'rb') as f:
    ...
    for hh in range(t):
        ...
        ads = [(0, 0)] + [(int(input()), int(input())) for _ in range(n)]
        answer = 0
        for _ in range(v):
            a1, a2, c = (int(input()) for _ in range(3))
            if ads[a1][0] == 0 and c == 1:
                answer += ads[a1][1]
            elif ads[a1][0] == 1:
                answer += ads[a1][1]
            if ads[a2][0] == 0 and c == 2:
                answer += ads[a2][1]
            elif ads[a2][0] == 1:
                answer += ads[a2][1]
        answers.append(f"Data Set {hh+1}:\n{answer}\n")
    print(*answers)
