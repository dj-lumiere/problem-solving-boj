...
with open(0, 'r') as f:
    ...
    for hh in range(1, t + 1):
        ...
        pressed_chars = set(input())
        answer = ""
        for i, j in product(range(4), range(10)):
            if keyboard[i][j] not in pressed_chars:
                continue
            neighbors = {keyboard[i][j]}
            for dx, dy in DELTA:
                nx, ny = i + dx, j + dy
                if not is_inbound(nx, 4, ny, 10):
                    continue
                neighbors.add(keyboard[nx][ny])
            if neighbors == pressed_chars:
                answer = keyboard[i][j]
                break
