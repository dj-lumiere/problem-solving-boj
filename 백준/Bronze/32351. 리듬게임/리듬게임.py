...
with open(0, 'r') as f:
    ...
    for hh in range(1, t + 1):
        ...
        speed_changes = [(1, s0)]
        for _ in range(k):
            mi, si = int(input()), float(input())
            speed_changes.append((mi, si))
        speed_changes.append((n+1, speed_changes[-1][1]))
        eprint(speed_changes)
        for (s, b), (e, _) in zip(speed_changes, speed_changes[1:]):
            answer += (e-s)*4*(60/b)
            eprint(answer)
