...
with open(0, 'rb') as f:
    ...
    for hh in range(t):
        ...
        for i, letters in enumerate(zip_longest(*words, fillvalue=""), start=1):
            letter_count = Counter(letters)
            del letter_count[""]
            most_frequent_items = sorted([item for item, count in letter_count.items() if
                                          count == max(letter_count.values())])
            answers.append(f"{i}: {' '.join(most_frequent_items)}")
    print(*answers)