...
with open(0, 'rb') as f:
    ...
    for hh in range(t):
        ...
        possible = set(i + j + k for i, j, k in
                       product(map(lambda x: x * a, range(-1, 2)), map(lambda x: x * b, range(-1, 2)), map(lambda x: x * c, range(-1, 2)))
                       if not (i == j == k == 0))
        answer = "S" if 0 in possible else "N"
