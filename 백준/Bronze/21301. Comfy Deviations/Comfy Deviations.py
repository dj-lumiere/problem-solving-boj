...
with open(0, 'r') as f:
    ...
    for hh in range(1, t + 1):
        temps = [float(input()) for i in range(10)]
        total_sum = sum(temps)
        mean_temp = total_sum / 10
        sum_of_squares = sum([(v - mean_temp) ** 2 for v in temps])
        std_dev = sqrt(sum_of_squares / (10 - 1))
        if std_dev <= 1.0:
            answers.append("COMFY")
        else:
            answers.append("NOT COMFY")
    print(*answers, sep="\n")