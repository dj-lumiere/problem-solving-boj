...
with open(0, 'r') as f:
    ...
    for hh in range(1, x + 1):
        N = int(input())
        petals = [int(input()) for _ in range(N)]
        count_photos_with_avg_flower = 0
        for i in range(N):
            for j in range(i, N):
                total = 0
                flower_set = set()
                for k in range(i, j + 1):
                    total += petals[k]
                    flower_set.add(petals[k])
                average = total / (j - i + 1)
                if average in flower_set:
                    count_photos_with_avg_flower += 1
        answers.append(count_photos_with_avg_flower)
    print(*answers, sep="\n")