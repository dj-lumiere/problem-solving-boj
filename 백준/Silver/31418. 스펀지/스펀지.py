...
with open(0, 'r') as f:
    ...
    for hh in range(1, t + 1):
        w, h, k, tt = (int(input()) for _ in range(4))
        viruses = [(int(input()), int(input())) for _ in range(k)]
        movable_points = [(max(1, x - tt), max(1, y - tt), min(w, x + tt), min(h, y + tt)) for x, y in viruses]
        answer = 1
        for x1, y1, x2, y2 in movable_points:
