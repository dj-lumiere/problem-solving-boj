...
with open(0, 'r') as f:
    ...
    t = 1
    answers = []
    leets = {"0": "O", "1": "L", "2": "Z", "3": "E", "5": "S", "6": "B", "7": "T", "8": "B"}
    for hh in range(t):
        n, m = int(input()), int(input())
        bad_words = [input() for _ in range(n)]
        target = [input() for _ in range(m)]
        for i, v in enumerate(target):
            leet_convert = "".join([leets.get(v2, v2) for v2 in v])
            answer = "INVALID" if any(j in leet_convert for j in bad_words) else "VALID"
