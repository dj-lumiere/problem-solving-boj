...
with open(0, 'r') as f:
    ...
    for hh in range(1, x + 1):
        xs, ys, xe, ye, dx, dy = (int(input()) for _ in range(6))
        on_line = lambda x, y: ((y - ye) * dx == dy * (x - xe)) and (y - ye) * dy >= 0 and (x - xe) * dx >= 0
        min_distance = INF
        min_point = (INF, INF)
        for x, y in product(range(-100, 101), repeat=2):
            if not on_line(x, y):
                continue
            ...
            if distance < min_distance:
                ...
                min_point = (x, y)
        answers.append(f"{min_point[0]} {min_point[1]}")
    print(*answers, sep="\n")