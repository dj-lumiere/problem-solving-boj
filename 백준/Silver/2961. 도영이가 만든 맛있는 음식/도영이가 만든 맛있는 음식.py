...
with open(0, 'r') as f:
    ...
    for hh in range(t):
        n = int(input())
        ingredients = [(int(input()), int(input())) for _ in range(n)]
        minimum_difference = INF
        for mask in range(1, 1 << n):
            contained = [mask & (1 << i) != 0 for i in range(n)]
            sourness = 1
            bitterness = 0
            for i, (s, b) in enumerate(ingredients):
                if contained[i]:
                    sourness *= s
                    bitterness += b
            minimum_difference = min(minimum_difference, abs(sourness - bitterness))
        answer = minimum_difference
