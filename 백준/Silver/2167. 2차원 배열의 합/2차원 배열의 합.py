...
class SegmentTree:
    ...
    def __init__(self, members: list[int], base_index: int = 0):
        ...
        self.index_offset = self.tree_capacity - self.starting_index
        self.build(members)

    def build(self, members: list[int]):
        for index, member in enumerate(members, start=self.index_offset):
            self.tree[index] = member
...
with open(0, 'rb') as f:
    ...
    n = int(input())
    m = int(input())
    grid = [[]] + [SegmentTree([0] + [int(input()) for _ in range(m)], base_index=1) for _ in range(n)]
    t = int(input())
    for hh in range(t):
        i, j, x, y = [int(input()) for _ in range(4)]
        answer = 0
        for k in range(i, x + 1):
            answer += grid[k].query(j, y)
