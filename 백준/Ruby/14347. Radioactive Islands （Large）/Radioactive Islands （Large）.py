...
def runge_kutta(
    ...
    try:
        for _ in range(int(X_START / STEP), int(X_END / STEP) + 1):
            ...
            dose += (
                STEP
                * dose_rate(island_y_pos, boat_x_pos, boat_y_pos)
                * curve_length_part(boat_y_pos_prime) ** 0.5
            )
            runge_kutta_1st_const_k = STEP * boat_y_pos_prime
            runge_kutta_1st_const_l = STEP * find_y_double_prime(
                island_y_pos, boat_x_pos, boat_y_pos, boat_y_pos_prime
            )
            runge_kutta_2st_const_k = STEP * (
                boat_y_pos_prime + runge_kutta_1st_const_l / 2.0
            )
            runge_kutta_2st_const_l = STEP * find_y_double_prime(
                island_y_pos,
                boat_x_pos + STEP / 2.0,
                boat_y_pos + runge_kutta_1st_const_k / 2.0,
                boat_y_pos_prime + runge_kutta_1st_const_l / 2.0,
            )
            runge_kutta_3st_const_k = STEP * (
                boat_y_pos_prime + runge_kutta_2st_const_l / 2
            )
            runge_kutta_3st_const_l = STEP * find_y_double_prime(
                island_y_pos,
                boat_x_pos + STEP / 2,
                boat_y_pos + runge_kutta_2st_const_k / 2,
                boat_y_pos_prime + runge_kutta_2st_const_l / 2,
            )
            runge_kutta_4st_const_k = STEP * (
                boat_y_pos_prime + runge_kutta_3st_const_l
            )
            runge_kutta_4st_const_l = STEP * find_y_double_prime(
                island_y_pos,
                boat_x_pos + STEP,
                boat_y_pos + runge_kutta_3st_const_k,
                boat_y_pos_prime + runge_kutta_3st_const_l,
            )
            boat_x_pos += STEP
            boat_y_pos += (
                runge_kutta_1st_const_k
                + 2 * runge_kutta_2st_const_k
                + 2 * runge_kutta_3st_const_k
                + runge_kutta_4st_const_k
            ) / 6
            boat_y_pos_prime += (
                runge_kutta_1st_const_l
                + 2 * runge_kutta_2st_const_l
                + 2 * runge_kutta_3st_const_l
                + runge_kutta_4st_const_l
            ) / 6
    finally:
...
def binary_search(
    ...
    while abs(right_y_prime - left_y_prime) > ERROR_LIMIT:
        ...
        result, boat_y_pos, boat_x_pos = runge_kutta(
            island_y_pos, X_START, A, mid_y_prime
        )
        if boat_y_pos >= B:
...
def find_minimal_dose(A: float, B: float, island_y_pos: list[float]):
    ...
    for index, slope in enumerate(slope_list):
        if index == 0:
            continue
        result_sub, boat_y_pos, boat_x_pos = binary_search(
            A, B, island_y_pos, slope_list[index - 1], slope
        )
