...
segment_tree = SegmentTree(member=numbers, default=0, identity=0, index_start=0)
m = int(input())
for _ in range(m):
    opcode, *operand = map(int, input().split(" "))
    if opcode == 1:
        start, end, value = operand
        segment_tree.range_update(left=start, right=end, value=value)
    if opcode == 2:
        start, end = operand
        print(f"{segment_tree.range_query(left=start, right=end)}\n")