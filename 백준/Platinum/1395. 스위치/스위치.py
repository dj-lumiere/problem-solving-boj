...
class SegmentTree:
    ...
    def update_node(self, lazy_value, node_value, lazy_multiplier):
        # Change this part for various operations
        return lazy_multiplier - node_value if lazy_value else node_value
...
numbers = [0] * n
segment_tree = SegmentTree(member=numbers, default=0, identity=0, index_start=1)
answers = []
for i in range(m):
    opcode = int(next(tokens))
    if opcode == 0:
        start, end = int(next(tokens)), int(next(tokens))
        segment_tree.range_update(left=start, right=end, value=1)
    if opcode == 1:
        start, end = int(next(tokens)), int(next(tokens))
        answers.append(f"{segment_tree.range_query(left=start, right=end)}")
os.write(1, "\n".join(answers).encode())