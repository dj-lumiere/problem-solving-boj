...
class SegmentTree:
    ...
    def query(self, index_start, index_end, k):
        ...
        while start + 1 < end:
            mid = (start + end) // 2
            index_start2 = index_start + self.tree_capacity - 1
            index_end2 = index_end + self.tree_capacity - 1
            result = 0
            while index_start2 <= index_end2:
                if index_start2 % 2 == 1:
                    result += bisect_right(self.tree[index_start2], mid)
                    index_start2 += 1
                if index_end2 % 2 == 0:
                    result += bisect_right(self.tree[index_end2], mid)
                    index_end2 -= 1
                index_start2 >>= 1
                index_end2 >>= 1
            if result >= k:
