    ...
    def dfs(self, current_node):
        ...
        for next_node in self.graph[current_node]:
            ...
            if self.on_stack[next_node]:
                self.low[current_node] = min(self.low[current_node], self.low[next_node])
...
def sol():
    ...
    edges = [tuple(map(int, input().split())) for _ in range(M)] + [(1, 1)]
    for u, v in edges:
    ...
    for i in range(1, N + 1):
        if tarjan.scc_index[convert_if_negative(i)] == tarjan.scc_index[convert_if_negative(-i)]:
            return "no"
    return "yes"
