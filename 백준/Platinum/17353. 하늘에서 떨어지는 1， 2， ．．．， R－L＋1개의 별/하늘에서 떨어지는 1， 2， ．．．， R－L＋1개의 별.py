...
for i, v in enumerate(numbers):
    if i == 0:
        continue
    numbers2.append(v - numbers[i - 1])
M = int(input())
my_segment_tree = SegmentTree(member=numbers2, default=0, identity=0, index_start=1)
...
for _ in range(M):
    ...
    # Update operation
    # 계차수열로 업데이트를 했기 때문에 [L, R]애 +1
    if opcode == 1:
        start, end = operand
        my_segment_tree.range_update(left=start, right=end, value=1)
        # R이 배열의 끝이 아니면 R+1번째에 -(R-L+1)
        if end != N:
            my_segment_tree.point_update(index=end + 1, value=-(end - start + 1))

    # Query operation
    # X까지의 합을 구하면 X에 떨어진 실제 별의 갯수가 나옴
    if opcode == 2:
        point = operand[0]
        print(my_segment_tree.range_query(left=1, right=point))
