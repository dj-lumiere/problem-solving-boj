...
class SegmentTree:
    ...
    def update_node(self, lazy_value, node_value, lazy_multiplier):
        ...
        if lazy_value == self.identity:
            return node_value
        return lazy_value * lazy_multiplier

    def compose_lazies(self, lazy_value1, lazy_value2):
        # Change this part for various operations
        return lazy_value2 if lazy_value1 == self.identity else lazy_value1
...
member_order = [0] * (N + 1)
segment_tree = SegmentTree(members=[1] * N, default=0, identity=-1, index_start=1)
# Process the operations
for _ in range(M):
    opcode, *operand = list(map(int, input().split()))
    starting_member = operand[0]
    #print(f"{opcode} {operand[0]} {(time_in[starting_member], time_out[starting_member])}\n")
    # Update operation
    if opcode == 1:
        start = time_in[starting_member]
        end = time_out[starting_member]
        current_state = segment_tree.point_query(start)
        #print(f"{segment_tree}\n")
        segment_tree.range_update(start, end, 1)
        #print(f"{segment_tree}\n")
        segment_tree.point_update(start, current_state)
        #print(f"{segment_tree}\n")
    ...
    if opcode == 2:
        ...
        current_state = segment_tree.point_query(start)
        #print(f"{segment_tree}\n")
        segment_tree.range_update(start, end, 0)
        #print(f"{segment_tree}\n")
        segment_tree.point_update(start, current_state)
        #print(f"{segment_tree}\n")
    ...
    if opcode == 3:
        ...
        current_state = segment_tree.point_query(start)
        #print(f"{segment_tree}\n")
        print(f"{segment_tree.range_query(start, end) - current_state}\n")
        #print(f"{segment_tree}\n")