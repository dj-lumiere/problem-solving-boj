...
class SegmentTreeBeats:
    ...
    def range_divisors(self, l, r):
        """Divide by x to elements in [l, r)."""
        cmp1 = lambda k: self.max_value[k] <= 2
        cmp2 = lambda k: divisors[self.max_value[k]] - self.max_value[k] == divisors[self.min_value[k]] - \
                         self.min_value[k]
        apply_func = lambda k: self._apply_add(k, divisors[self.max_value[k]] - self.max_value[k])
        self._range_operation(l, r, cmp1, cmp2, apply_func)
...
def division_count(value: int, prime_list: list[int]) -> int:
    prime_factors = get_prime_factors(value, prime_list)
    factorized_dict = Counter(prime_factors)
    result = 1
    for v in factorized_dict.values():
        result *= (v + 1)
    return result
...
with open(0, 'r') as f:
    ...
    divisors = [0, 1] + [1 for i in range(2, 10 ** 6 + 1)]
