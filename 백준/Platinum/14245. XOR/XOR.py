...
class SegmentTree:
    ...
    def update_node(self, lazy_value, node_value, lazy_multiplier):
        if lazy_multiplier & 1:
            return node_value ^ lazy_value
        return node_value
...
my_lazy_segment_tree = SegmentTree(member=numbers, default=0, identity=0, index_start=0)
m = int(input())
for _ in range(m):
    opcode, *operand = map(int, input().split(" "))
    if opcode == 1:
        start, end, value = operand
        my_lazy_segment_tree.range_update(left=start, right=end, value=value)
    if opcode == 2:
        point = operand[0]
        print(f"{my_lazy_segment_tree.point_query(point)}\n")