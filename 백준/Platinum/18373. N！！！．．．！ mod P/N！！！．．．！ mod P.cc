...
list<int64> multiply_fft(list<int64> &v, list<int64> &w, int64 mod)
{
    ...
    for (int64 i = 0; i < n; i++)
    {
        int128 result_high_digit = round(result_sub1[i].real());
        int128 result_mid_digit = round(result_sub1[i].imag()) + round(result_sub2[i].real());
        int128 result_low_digit = round(result_sub2[i].imag());
        result_high_digit %= mod;
        result_mid_digit %= mod;
        result_low_digit %= mod;
        result1[i] = (int64)(((result_high_digit << 30) + (result_mid_digit << 15) + result_low_digit) % mod);
...
list<int64> lagrange(list<int64> &h, int64 mod)
{
    ...
    /* part 1 : necessary precalculations (factorial (i!), inverse factorial (i!**-1), inverse number(i**-1)) */
    ...
    /* part 3 : 1/(i!(dimension-i)!(-1)^(dimension-i)) */
    ...
    /* part 4 : multiple_fft(part 2, inverse_number) */
...
int64 factorial_fft(int64 N, int64 P)
{
    int64 d = 1;
    int64 answer = 1;
...
int32 main()
{
    ...
    int32 N, K, P;
    input >> N >> K >> P;
    list<int32> factorial_list = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600};
    if (N >= P)
    {
        print << 0;
    }
    ...
    else if (K == 3 and N == 3 and P <= 720)
    {
        print << 0;
    }
    else if (K == 3 and N == 3 and P > 720)
    {
        print << factorial_fft(720, P);
    }
    else if (K == 2 and N > 12)
    {
        print << 0;
    }
    else if (K == 2 and N <= 12 and factorial_list[N] >= P)
    {
        print << 0;
    }
    else if (K == 2 and N <= 12 and factorial_list[N] < P)
    {
        print << factorial_fft(factorial_list[N], P);
    }
}
