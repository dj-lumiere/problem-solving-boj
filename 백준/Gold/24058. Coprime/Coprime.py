...
for i in range(1, len(prime_factors) + 1):
    for prime_factors_sub in combinations(prime_factors, i):
        step = reduce(lambda x, y: x * y, prime_factors_sub, 1)
        removal_number_sum = n * k * (n * k + step) // 2 // step
        result += (-1) ** i * removal_number_sum
print(result)