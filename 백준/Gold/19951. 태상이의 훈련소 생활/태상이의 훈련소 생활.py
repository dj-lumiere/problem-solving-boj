...
with open(0, 'r') as f:
    ...
    for hh in range(1, t + 1):
        n = int(input())
        m = int(input())
        grid = [0] + [int(input()) for _ in range(n)]
        grid_rectangle = [0 for _ in range(n + 2)]
        for _ in range(m):
            a, b, k = (int(input()) for _ in range(3))
            grid_rectangle[b + 1] -= k
            grid_rectangle[a] += k
        for i in range(1, n + 2):
            grid_rectangle[i] += grid_rectangle[i - 1]
        answer = " ".join(map(str, [i + j for i, j in zip(grid[1:], grid_rectangle[1:])]))
