...
parity = lambda x: int(x) % 2
N = int(stdin.readline().strip())
my_segment_tree = SegmentTree([parity(i) for i in stdin.readline().strip().split(" ")])
M = int(stdin.readline().strip())
for _ in range(M):
    operator, *operand = (int(i) for i in stdin.readline().strip().split(" "))
    if operator == 1:
        operand[1] = parity(operand[1])
        my_segment_tree.update(*operand)
    elif operator == 2:
        query_result = operand[1] - operand[0] + 1 - my_segment_tree.query(*operand)
        stdout.writelines(str(query_result) + "\n")
    elif operator == 3:
        query_result = my_segment_tree.query(*operand)
        stdout.writelines(str(query_result) + "\n")
