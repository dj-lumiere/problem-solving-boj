...
class SegmentTree:
    ...
    def __init__(self, members: list[int]):
        ...
        self.tree = [INF for _ in range(self.tree_capacity << 1)]
        self.build(members)
...
A = [INF] * (L - 1) + list(map(int, input().split()))
st = SegmentTree(A)
# print(st.__repr__())
# print([A[i:i + L] for i in range(N)])
answer = [st.query(i + 1, i + L) for i in range(N)]
print(*answer)