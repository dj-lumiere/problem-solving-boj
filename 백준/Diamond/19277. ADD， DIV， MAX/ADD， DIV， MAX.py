...
class SegmentTreeBeats:
    ...
    def _apply_lower_clamping(self, k, x):
    ...
    def _apply_upper_clamping(self, k, x):
    ...
    def _apply_division(self, k, x):
        """divide elements in node k by x."""
        val = self.max_value[k] // x
        length = self.right_bound[k] - self.left_bound[k]
    ...
    def _range_operation(self, l, r, cmp1, cmp2, apply_func):
    ...
    def range_lower_clamping(self, l, r, x):
    ...
    def range_upper_clamping(self, l, r, x):
    ...
    def range_divide(self, l, r, x):
        """Divide by x to elements in [l, r)."""
        cmp1 = lambda k: self.max_value[k] == 0 or x == 1
        cmp2 = lambda k: self.max_value[k] // x - self.max_value[k] == self.min_value[k] // x - self.min_value[k]
        apply_func = lambda k: self._apply_add(k, self.max_value[k] // x - self.max_value[k])
        self._range_operation(l, r, cmp1, cmp2, apply_func)
    ...
    def _range_query(self, l, r, initial_value, compare_func):
    ...
    for hh in range(1, t + 1):
        ...
        for _ in range(m):
            ...
            if q == 0:
                ...
                # frprint(q, l, r, x, file=g)
                stb.range_add(l, r + 1, x)
                # frprint(*print_attributes(stb), sep="\n", file=g)
            if q == 1:
                l, r, x = (int(input()) for _ in range(3))
                # frprint(q, l, r, file=g)
                stb.range_divide(l, r + 1, x)
                # frprint(*print_attributes(stb), sep="\n", file=g)
            if q == 2:
                l, r, x = (int(input()) for _ in range(3))
                # frprint(q, l, r, file=g)
                answer = stb.get_max(l, r + 1)
