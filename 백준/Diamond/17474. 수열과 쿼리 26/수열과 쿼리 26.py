...
class SegmentTreeBeats:
    ...
    def __init__(self, n: int, iterable) -> None:
        ...
        self.tree_size = 1 << self.log  # Total number of leaves
        total_size = self.tree_size * 2  # Total number of nodes
        ...
        self.left_bound = [0] * (total_size)  # Left index of segment
        self.right_bound = [0] * (total_size)  # Right index of segment

        self._build_tree(iterable)

    def _build_tree(self, iterable):
        size = self.tree_size
        # Build the segment tree structure
        for i, (v1, v2) in enumerate(zip(range(size), range(1, size + 1)), start=size):
            self.left_bound[i] = v1
            self.right_bound[i] = v2
    ...
    def _propagate(self, k: int):
        ...
        # Propagate min and max constraints of left child
        ...
        # Propagate min and max constraints of right child
    ...
    def _apply_update(self, k: int, x):
        ...
        # max and min values are k, and there are no penultimate values.
    ...
    def _range_operation(self, l: int, r: int, skip_condition, apply_condition, apply_func) -> None:
        """Generic method for applying operations over a range."""
        stack, order = [1], []
        while stack:
            k = stack.pop()
            if r <= self.left_bound[k] or self.right_bound[k] <= l or skip_condition(k):
                continue
            if l <= self.left_bound[k] and self.right_bound[k] <= r and apply_condition(k):
                apply_func(k)
                continue
            order.append(k)
            self._propagate(k)
    ...
    def range_lower_clamping(self, l: int, r: int, x: int) -> None:
        ...
        skip_condition = lambda k: self.max_value[k] <= x
        apply_condition = lambda k: self.second_max_value[k] < x
        apply_func = lambda k: self._apply_lower_clamping(k, x)
        self._range_operation(l, r, skip_condition, apply_condition, apply_func)

    def range_upper_clamping(self, l: int, r: int, x: int) -> None:
        """Update elements in [l, r) to max(x, A[i])."""
        skip_condition = lambda k: x <= self.min_value[k]
        apply_condition = lambda k: x < self.second_min_value[k]
        apply_func = lambda k: self._apply_upper_clamping(k, x)
        self._range_operation(l, r, skip_condition, apply_condition, apply_func)

    def range_add(self, l: int, r: int, x: int) -> None:
        """Add x to elements in [l, r)."""
        skip_condition = lambda _: False
        apply_condition = lambda _: True
...
with open(0, 'r') as f:
    ...
    rprint = lambda *args, sep="\n", end="\n": stdout.write(sep.join(map(repr, args)) + end)
    ...
    for hh in range(1, t + 1):
        ...
        for _ in range(m):
            ...
            if q == 1:
                ...
                stb.range_lower_clamping(l - 1, r, x)
            if q == 2:
                l, r = (int(input()) for _ in range(2))
                answer = stb.get_max(l - 1, r)
