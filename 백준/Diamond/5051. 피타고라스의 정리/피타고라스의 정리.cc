...
vec<int64> square_fft(vec<int64> &a)
{
    ...
    for (int64 i = 0; i < n; i++)
    {
        transformed_a[i] *= transformed_a[i];
    }
...
int32 main()
{
    ...
    vec<int64> answer_sub = square_fft(a);
    int64 answer = 0;
    for (int64 i = 1; i < N; i++)
    {
        int64 target = i * i % N;
        answer += answer_sub[target];
        answer += answer_sub[target + N];
        // 2로 나누기 전 a=b인 케이스 미리 더하기
        answer += c[target];
    }
    answer /= 2;
    print << answer << "\n";
