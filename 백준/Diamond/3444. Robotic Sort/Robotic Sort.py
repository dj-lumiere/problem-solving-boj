...
class SplayTree:
    ...
    def __getitem__(self, item):
        self.kth_to_root(item)
        return self.root.key

    def __iter__(self):
        return iter([self[i] for i in range(1, len(self.tree) - 1)])
...
def main():
    with open(0, 'r') as f:
        ...
        for hh in range(t):
            ...
            if n == 0:
                break
            a = [(int(input()) << 20) + i for i in range(1, n + 1)]
            a_index = {v: i for i, v in enumerate(a)}
            a.sort()
            sorted_a = [0 for _ in range(n)]
            for i, v in enumerate(a, start=1):
                sorted_a[a_index[v]] = i
            st.build_tree(n, sorted_a)
            answer = []
            for i in range(1, n + 1):
                idx = st.index(i)
                answer.append(idx)
                st.reverse_range(i, answer[-1])
